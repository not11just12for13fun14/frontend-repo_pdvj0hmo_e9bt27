      ...
      <div className="max-w-md mx-auto px-4 pt-24">
        ...
        <form onSubmit={submit} className="bg-slate-800/60 border border-slate-700 rounded-lg p-6 space-y-4">
          ...
          <div>
            ...
            <input value={name} onChange={e=>setName(e.target.value)} required className="w-full bg-slate-900 text-white rounded px-3 py-2 border border-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
          ...
          <button className="w-full bg-blue-600 hover:bg-blue-500 text-white rounded px-3 py-2">Create account</button>
          <div className="text-blue-300 text-sm text-center">Already have an account? <Link to="/login" className="text-white underline">Sign in</Link></div>
        </form>
      </div>
    </div>
  )
